...
def process_order(request, checkoutForm):
    ...
    print("HERE")
    print(order.shipping_type)
    print(order.shipping_total)
...
def validate_form_stripe(request):
    ...
    print(request.POST.get('postage'))
...
def valid_delivery_date():
    two_thirty = datetime.now().replace(hour=14, minute=30, second=0, microsecond=0)
    time_now = datetime.now().time()

    min_date = date.today() + timedelta(days=4)
    # After 2:30pm it takes 4 days to deliver
    if two_thirty.time() <= time_now:
...
def cart_changed_state(request):
    ...
    print("New location: " + str(request.session['state_selected']))
...
def cart_quantity_change(request):
    ...
    if request.GET.get('subtract') == 'true':
        if cart_item.quantity > 1:
            ...
            cart_obj.total = round(
                Decimal(cart_obj.total) - Decimal(cart_item.price), 2)
            cart_obj.save()
        ...
        cart_obj.total = round(Decimal(cart_obj.total) +
                               Decimal(cart_item.price), 2)
        cart_obj.save()
...
def remove_product(request, product_id, state_selected):
    ...
    cart_obj.total -= round(Decimal(cart_item.price) * Decimal(cart_item.quantity), 2)
