$(document).ready(function () {
  ...
  $.fn.check_button_state = function () {
    ...
    if ($("#length").length) {
      ...
      if (length != "Choose Length" && length != "") {
        lengthCheck = 1;
        if ($("#quantity").length) {
  ...
  if ($("#product_row").children().length == 0) {
    $("#product_row").append(
      "<div class='row'><div class='col text-center mb-5'><h1>Sorry, we are unable to supply this product to your location. We are always working on new delivery areas.</h1></div></div>"
    );
  }
